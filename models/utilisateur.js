class Utilisateur {
    ...
    static async getById(id) {
        try {
            ...
            if (userData) {
                const user = new Utilisateur(); // Crée une instance vide de user
    ...
    async save() {
        try {
            const lastUserId = await Utilisateur.#getLastUserId();
...
async function createU() {
    ...
    const newUser = new Utilisateur(username, password, email, role);
...
async function displayUsers() {
    ...
    users.forEach((user) => {
        ...
        deleteButton.onclick = async () => {
            const userToDelete = await Utilisateur.getById(user.id);
